 ...
 *    的比较结果为0，即比较关键字相等，则将会发生覆盖value，但是key不变的情况，具体如下：
 *      1. 如果按照Key来排序，则相同的Key的Value值将会发生覆盖，即value值等于最近put方法的指定的value值，Key不变；
 *      2. 如果按照Value来排序，则相同的Value的key将会发生覆盖，注意和1的区别：key还是原来的key,value还是原来的value
 *         只是进行了一次没有意义的value = value 的等值赋值的过程。
 *
 * */
